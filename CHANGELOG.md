...
- Replace the HTTP response cache with a logical, domain-aware cache. The cache skips bodies over 10MB (configurable with the `api_cache_max_file_size` setting) ([#248](https://github.com/antithesishq/snouty/issues/248)). A cache hit no longer authenticates the request first, so cached reads work without valid credentials ([#269](https://github.com/antithesishq/snouty/issues/269))
