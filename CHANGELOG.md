...
- Converge the human output of `runs logs`, `runs events`, and `runs search` on one event-aware renderer. Each event is rendered as a concise colored block. A new `-d/--detail` flag on all three commands adds full-precision vtimes, source locations, attached details JSON, and the composer's captured stdout/stderr. `runs build-logs` shares the same visual grammar. ([#222](https://github.com/antithesishq/snouty/pull/222))
