...
- The response cache now respects the server's cache headers: a response enters the cache only when its `Cache-Control` allows the client to cache it, on top of snouty's own admission checks. Set `api_cache_respect_headers = false` (settings file) or `SNOUTY_API_CACHE_RESPECT_HEADERS=false` to drop the header requirement if a tenant's headers prove faulty ([#263](https://github.com/antithesishq/snouty/issues/263))
