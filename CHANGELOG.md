...
- `snouty launch` validates `antithesis.filter_logs_matching` client-side before any API call: it rejects an empty pattern, a pattern over 1023 bytes, an invalid regex (with an RE2 note for lookaround and backreferences), and a pattern that matches the empty string. A JS-style `/pattern/flags` value launches with a warning that suggests the inline-flag rewrite ([#276](https://github.com/antithesishq/snouty/pull/276))
