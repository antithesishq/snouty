...
- Make VTIME optional on `snouty runs logs`: omit it to stream to the branch's current end. When VTIME is given, snouty ends the stream at that moment — the API streams past it — and caches the truncated response; a request without VTIME is not cached ([#264](https://github.com/antithesishq/snouty/issues/264))
