...
- Replace the HTTP response cache with a logical, domain-aware cache: immutable resources (logs, properties, terminal run details, fully streamed build logs) are cached forever, everything else is never cached, and any cache error falls back to the server. The cache lives under `$XDG_RUNTIME_DIR/snouty/api-cache-v2` (under the user temp dir on macOS) and skips bodies over 10MB (configurable with the `api_cache_max_file_size` setting, which accepts a size such as `"20 MB"` or a bare byte count) ([#248](https://github.com/antithesishq/snouty/issues/248))
