...
- `snouty runs events` matches a term against the text an event carries: log output, an assertion's message and source function, and a test-composer command. The `runs-search` unstable feature no longer changes which events a term matches ([#253](https://github.com/antithesishq/snouty/pull/253))
