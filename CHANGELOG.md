...
- `snouty doctor --json` writes `settings` as an object keyed by setting name, and writes an unset setting as `null` instead of a human sentinel like `not set`. A caller reads one value with `jq .settings.tenant`. This is a breaking change for anything that read `settings` ([#285](https://github.com/antithesishq/snouty/pull/285), [#288](https://github.com/antithesishq/snouty/pull/288))
