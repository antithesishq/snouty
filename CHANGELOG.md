...
- `snouty runs events` now filters entirely server-side and prints one `HASH VTIME SOURCE OUTPUT` line per event as it arrives, replacing the buffered table (no header row, no column padding, no output truncation). Multiple `--match` needles require the events-search API (behind the `runs-search` unstable feature); without it the command fails with the fix instead of approximating the match client-side. `--limit 0` is rejected up front ([#206](https://github.com/antithesishq/snouty/pull/206))
