...
`private_registries` lists registries, or repository paths below a registry, that need your credentials. `snouty launch --config` never pins a compose service image below a listed prefix at its own address. It copies the image into your repository instead, so the test run pulls from a registry it can reach. An image at a private registry that is not listed keeps its own address in the pin, because your container engine confirms the image with your credentials, and the test run then fails to pull it. In a settings file the value is a TOML array; in the environment variable, separate the entries with commas. snouty does not pull the image itself: see [the cookbook](COOKBOOK.md#launch-with-images-from-a-private-registry).
