...
- Converge the human output of `runs logs`, `runs events`, and `runs search` on one event-aware renderer. Each event is auto-classified (SDK assertions, guidance, SDK handshake, setup-complete, faults, container lifecycle, test-composer chatter, plain log text, unknown JSON) and rendered as a concise colored block: a `moment HASH VTIME` divider (full precision, pasteable into `runs logs`/`runs exec`/`snouty debug`) opens each timeline segment, and each event renders as one `VTIME [source] payload` line. Assertions carry a verdict badge — HIT/FAIL on evaluations, FAIL only on a failing always/unreachable, CATALOG on the SDK's startup registrations. SUT color codes survive in log text (restricted to SGR so they cannot corrupt the terminal). A new `-d/--detail` flag on all three commands adds full-precision vtimes, source locations, attached details JSON, and the composer's captured stdout/stderr. Source labels past 40 columns (generated k8s pod names) truncate with the full value on a dim detail line. `runs build-logs` shares the same visual grammar. `--raw` (now also on `runs events` and `runs search`) requires `--json` and passes the server's NDJSON through verbatim. Scripts should use `--json`, which is unchanged
